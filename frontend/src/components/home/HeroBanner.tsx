...
const HERO_SLIDES = [
	...
	},
	{
		...
		description: "Trải nghiệm và khám phá những sự kiện văn hóa, lễ hội hội xuân đặc trưng, kết tinh giá trị truyền thống Đồng Tháp Mười.",
...
export default function HeroBanner({ highlightStats }: HeroBannerProps) {
	...
	useEffect(() => {
		...
		}, 7000); //Thay đổi slide sau mỗi 7 giây.
			...
			<div className="grid items-center gap-10 md:grid-cols-2 md:gap-12">
				<div>
					<div className={`transition-opacity duration-300 ${isFading ? "opacity-0" : "opacity-100"}`}>
						<h1 className="py-2 text-5xl font-black leading-tight text-slate-900 md:text-7xl">
							{activeSlide.titlePrefix}
							<span className={`block transition-colors duration-500 ${activeSlide.textColor}`}>
					...
					<div className="mt-6 flex flex-wrap gap-4">
						<Link href="/dich-vu/tra-cuu" className={`rounded-full px-8 py-3.5 text-sm font-bold text-white shadow-lg transition hover:-translate-y-0.5 ${activeSlide.indicatorColor}`}>
							Tra cứu hồ sơ
						</Link>
						<Link href="/thu-vien" className="rounded-full border-2 border-slate-200 px-8 py-3.5 text-sm font-bold text-slate-700 transition hover:border-slate-300 hover:bg-slate-50">
							Khám phá văn hóa
						</Link>
					</div>
				...
				<div className="relative aspect-square w-full md:aspect-[4/3]">
					...
					<div className="relative z-10 h-full w-full rounded-[3rem] overflow-hidden shadow-2xl">
