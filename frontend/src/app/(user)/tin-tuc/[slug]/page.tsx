//! Trang chi tiết bài viết tin tức, hiển thị nội dung chi tiết của một bài viết được đồng bộ từ backend
"use client";
...
function resolveArticleUrl(slug: string) {
...
export default function TinTucSlugPage() {
 //! LƯU Ý 1: BẮT ĐỊA CHỈ (DYNAMIC ROUTING)
  //! Dùng hook useParams của Next.js để "tóm" lấy cái slug trên thanh địa chỉ trình duyệt (VD: nguoi-dung-vao-link /tin-tuc/khai-truong -> nó sẽ lấy được chữ "khai-truong").
  ...
  useEffect(() => {
    ...
    const loadArticlePage = async () => {
      try {
     //! LƯU Ý 2: CẤT LƯỚI GOM DỮ LIỆU (PROMISE.ALL)
        //! Không chỉ lấy chi tiết bài viết đó, mà lấy luôn cả: Mảng toàn bộ bài viết (để làm tin liên quan), Danh mục (để đếm số lượng), và Bình luận. Gom 4 API gọi chung 1 lần để tiết kiệm thời gian load trang. 
        const [nextArticle, nextArticles, nextCategories, nextComments] =
          await Promise.all([
            getArticleBySlug(slug),//! gọi API lấy chi tiết bài viết dựa trên slug, nếu có id thì gọi API lấy chi tiết, nếu lỗi thì trả về dữ liệu tóm tắt dò được từ list, tránh crash UI.
            getArticles(),
            getCategories(),
...
//! LƯU Ý 3: BỘ NÃO PHÂN TÍCH (USE-MEMO)
  //! Nguyên một dàn useMemo bên dưới dùng để "nhào nặn" dữ liệu lấy từ Lưu ý 2 thành các khối trên Sidebar mà không cần gọi thêm API nào khác:
  //! - categories: Tính toán xem mỗi danh mục có bao nhiêu bài viết (count).
...
//! - latestNews: Cắt lấy 3 bài viết mới nhất.
...
//! - relatedArticles: Lọc ra các bài có cùng danh mục với bài hiện tại.
  ...
  const currentIndex = useMemo(
    //! - previousArticle / nextArticle: Tìm ra bài trước và bài sau để làm nút chuyển bài.
  ...
  const nextArticle =
    currentIndex >= 0 && currentIndex < allArticles.length - 1
      ...
      : null;
//! LƯU Ý 4: PHƯƠNG ÁN CHỐNG SẬP TRANG (FALLBACK UI)
  //! Xử lý rất kỹ: Nếu đang tải thì hiện vòng xoay (Loading).
  if (isLoading) {
    return (
...
//!  Nếu người dùng gõ bậy bạ một cái link không tồn tại (!article), code không báo lỗi đỏ lòm mà vẽ ra một màn hình thân thiện "Bài viết đang cập nhật" kèm nút quay về trang chủ.
  if (!article) {
    return (
      <main className="mx-auto flex w-full max-w-[1200px] flex-1 flex-col gap-8 px-4 py-8 sm:px-6 lg:px-8">
        <nav
...
//! LƯU Ý 5: XỬ LÝ LINK CHIA SẺ MẠNG XÃ HỘI
//! Chỉ resolve URL tuyệt đối khi người dùng bấm nút share/copy, để SSR và client render cùng markup ban đầu.
        ...
        <article className="col-span-12 flex flex-col gap-6 lg:col-span-8">
          ...
          <div className="flex flex-wrap gap-3 py-1">
            <button
              ...
              onClick={() =>
                openShareUrl(
                  `https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(resolveArticleUrl(article.slug))}`,
                )
              }
            ...
            <button
              ...
              onClick={() =>
                openShareUrl(
                  `https://zalo.me/share?url=${encodeURIComponent(resolveArticleUrl(article.slug))}`,
                )
              }
            ...
            <button
              type="button"
              onClick={() => void copyLink(resolveArticleUrl(article.slug))}
